...
// Refuerza todas las verificaciones de roles:
...
export const isModeratorOrAdmin = async (req, res, next) => {
    try {
        const user = await User.findByPk(req.userId);
        const roles = await user.getRoles();
        const hasRole = roles.find((role) => ["admin", "moderator"].includes(role.name));
        if (userRole) {
            next();
            return;
        }

        res.status(403).json({ message: "Requiere rol de moderador o administrador" });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};